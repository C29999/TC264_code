...
- `encoder_measure_flag` 控制发车后的计时和平均速度统计；总里程始终按编码器累计，因此未发车手推也能测试元素距离。未发车时速度控制仍保持电机硬停。
- `stop_flog`（原拼写保留）表示停车/保护状态。
- `total_distance` 是左右编码器绝对计数之和；单轮标定值为 11485 count/m，车体距离必须除以 `2 * 11485` 取双轮平均。
...
- `CR_NONE -> CR_START`：参考上下角点补线方案，必须同时满足左右巡线触界/双边丢失、左右近端角点存在且两侧补线均有效；弯道仅短暂丢线或只出现一侧折角不会进入十字。
- `CR_START`：某侧远端角点存在时直接连接近端/远端角点；缺失时使用该侧近端角点之前的边界斜率向上延长，对应参考代码的四点、三点和两点补线。两条补边线的中点生成 `M0-M1`；入口丢失、张开超过 48 px 或两近端角点进入图像下方后转 `CR_ENTER`，0.9 m 未进入则撤销。
- `CR_ENTER`：只在原图二值图 `image_binary` 上列扫最长白列、行扫稳定路口边缘，控制优先跟踪动态出口/虚拟中线。进入至少 0.44 m 后，普通左右边线各不少于 25 点且第 5 点宽度 40~48 px 连续 3 帧即退出；对面入口回到底部则转 `CR_OUT`，1.3 m 超时强制转 `CR_OUT`。
- `CR_OUT`：普通边线宽度连续恢复 3 帧，或行驶 0.17 m 后回到 `CR_NONE` 并清除锁存补线。
- 十字识别、补线、出口扫描和退出判断均不读取逆透视图；逆透视仍作为其他元素/调试基础保留。
- 屏幕和 WiFi 始终保留普通边线调试信息；三条虚拟补线只在本次十字的锁存有效期内显示。下方鸟瞰图已移除，数据页显示 `cross_distance` 和 `CR` 状态。

### 7.2 上位机十字调试协议

- `$TMODE <0..2>`：发送普通寻线、预十字、十字中状态。
- `$CDIST <m>`：发送从本次四角点首次确认位置起算的双轮平均真实距离。
- 左补线、右补线和中线通过三条不同颜色的 boundary 发送，避免上位机按颜色键覆盖。
